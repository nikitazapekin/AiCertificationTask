...
#### 9.4: Merge `.mcp.json` (MCP Servers)

If the update includes a `.mcp.json` file:

```bash
# Check if new .mcp.json exists in update
if [ -f /tmp/accelerator-update/.mcp.json ]; then
  if [ -f .mcp.json ]; then
    # Merge: new servers are added, user's existing servers are preserved
    python3 .claude/skills/accelerator-update/scripts/merge_settings.py \
      .mcp.json \
      /tmp/accelerator-update/.mcp.json \
      .mcp.json.merged
    mv .mcp.json.merged .mcp.json
  else
    # No local .mcp.json, just copy
    cp /tmp/accelerator-update/.mcp.json .mcp.json
  fi
fi
```

**Merge rules for `.mcp.json`:**
- New MCP servers from the update are **added**
- User's existing MCP servers are **preserved** (user values take precedence)
- If a server exists in both, user's version wins (they may have customized env vars)

#### 9.5: Update MCP Registry

```bash
# Always overwrite the MCP registry (it's accelerator-managed metadata)
if [ -f /tmp/accelerator-update/.claude/mcp-registry.json ]; then
  cp /tmp/accelerator-update/.claude/mcp-registry.json .claude/mcp-registry.json
fi
```

#### 9.6: Detect New MCP Requirements

Compare old and new `mcp-registry.json` to identify newly added MCP servers:

```bash
python3 .claude/skills/accelerator-update/scripts/check_new_mcps.py \
  .claude/mcp-registry.json.old \
  .claude/mcp-registry.json
```

Before overwriting the registry in step 9.5, back up the old one:
```bash
cp .claude/mcp-registry.json .claude/mcp-registry.json.old 2>/dev/null || true
```

For each new MCP server found:
- Read its entry from `mcp-registry.json`
- If `requiresAuth: true`, display a clear **action required** message with `setupInstructions`
- If `requiresAuth: false`, just inform the user it was added

**Display format:**
```
New MCP servers added by this update:

  context7 - Up-to-date documentation and code examples for any library
    Setup: No setup required. Works out of the box via npx.

  github - GitHub integration for PRs, issues, etc.
    ⚠️  Requires configuration:
    Setup: Generate a GitHub personal access token and set GITHUB_TOKEN env var.
    Required env vars:
...
```bash
rm -f .claude/mcp-registry.json.old
```

#### 9.7: Update Version Files
